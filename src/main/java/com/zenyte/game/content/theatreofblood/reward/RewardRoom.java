    ...
    public void onLoad() {
        ...
        for (Player p : getRaid().getParty().getPlayers()) {
            if (p.getVariables().getTobBoost() > 0 || Constants.BOOSTED_TOB) {
                ...
                playerPoints *= (p.getMemberRank().eligibleTo(MemberRank.ZENYTE_MEMBER) ? 0.1 : p.getMemberRank().eligibleTo(MemberRank.ONYX_MEMBER) ? 0.09 : p.getMemberRank().eligibleTo(MemberRank.DRAGONSTONE_MEMBER) ? 0.08 : p.getMemberRank().eligibleTo(MemberRank.DIAMOND_MEMBER) ? 0.07 : p.getMemberRank().eligibleTo(MemberRank.RUBY_MEMBER) ? 0.07 : p.getMemberRank().eligibleTo(MemberRank.EMERALD_MEMBER) ? 0.06 : p.getMemberRank().eligibleTo(MemberRank.SAPPHIRE_MEMBER) ? 0.06 : 0.05);
